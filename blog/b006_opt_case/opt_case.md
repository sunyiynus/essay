...
我拿到程序，一个读取365天，每天三张地图的数据，这个数据大概50G大小。这个程序从365天，每天三张地图中读取数据合成一张数据地图。每张图大概是3000x5000像素点。
...
其中我需要考虑到我内存的大小，16G，内存8G可用，那么*8 x 365 x 3 x K=8G*，求出*K*就是一次可以载入多少pixel。
