...
#### 在matlab中优化计算量。
背景：
读博士的朋友找到我让我帮她跑一个matlab程序。她跑了几天这个程序没跑完。
我拿到程序+数据，这个数据50G。这个程序从365天，每天三张地图中读取数据合成一张数据地图。每张图大概是3000x5000像素点。

第一次拿到这个matlab的代码，写了些print看看运行速度。发现确实很慢。

![图示](image/image.png) 
我仔细分析原始代码，其中的计算并不复杂，并不应该耗费太多时间。那么耗时间，就是文件IO的问题。    
最后仔细分析，发现每次生成一个像素点，都需要打开关闭365*3次文件，计算一张图需要3000x5000个像素点，然后就需要365x3x3000x5000=1.6亿次打开文件IO的操作。      
找到优化点之后，为了减少**打开关闭文件**的系统操作，我觉得可以充分利用每次打开一个文件的操作，读取一整块pixel，而不是只读取1pixel。   
其中需要考虑到内存的大小，内存8G可用，那么8byte x 365 x 3 x K = 8G，求出K就是一次可以载入多少pixel。   
然后计算出K～= 100w Pixel，原图3000x5000=1500w，所以原图我可以划分成3x5块。
那么我每次读取一张图的100w个pixel一次性计算。我可以减少IO到365x3x15次。优化了IO次数6个数量级。最终我跑完整个数据只用了30min。
...
##### 思考：
1. 可以使用matlab提供的process并行计算的框架，可以更快计算完成。
2. 还可以使用数组cache命中特性加速数组读取速度。
